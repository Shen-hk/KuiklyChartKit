...
> 状态：P3-1 已将手势从 Canvas 迁移到普通触摸层，并抽出纯 Kotlin `ChartGestureController`；`commonTest` 已覆盖 pan 钳制、pinch 焦点、双击复位、tracker release 与非有限值防护。Android 真机手势、APK 构建与 iOS/OpenHarmony 证据待补，尚未标记为正式三端能力。  
...
| P3-2 | Crosshair 与区间框选（Brush） | P3-1 正式验收 | 两者默认关闭。Crosshair 复用 tracker 的 RenderPlan 命中坐标，可选垂直/水平参考线与聚合 Tooltip；Brush 仅能在绘图区长按拖拽启动，输出稳定的原始索引闭区间与聚合摘要，支持“缩放至选区”和清除。两者不得抢占单点点击、长按追踪或正常平移；实现为独立状态机，并补充命中、状态迁移和跨端录屏验证。 |
...
- `LineChart` 与 `AreaChart` 的触摸层以单指 X 位移平移；以两指距离计算缩放比例，并以两指中心附近的数据索引为锚点；双击在触摸层复位。所有范围保持原始索引。
...
- 已通过 `.\\gradlew :chartkit:compileKotlinJs`；待补 Android Debug APK 构建、Android 真机录屏，以及完整 JS/Android 单元回归。P3-2 在这些证据齐全前不进入正式实现。
...
- Brush 独立保存“待开始 / 拖拽中 / 已选择”状态，开始、更新、取消和提交均为纯 Kotlin 可测试转移；组件只负责将最终原始索引区间回调给调用方。
- 手势仲裁顺序固定为：明确 Brush 模式 > 双指 pinch > 已激活 tracker > 单指 pan > click。P3-2 只有在该顺序及平台录屏通过后才从候选升级。
