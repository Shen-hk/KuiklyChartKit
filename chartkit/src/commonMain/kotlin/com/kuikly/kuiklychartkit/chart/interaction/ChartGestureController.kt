...
internal class ChartGestureController {
    ...
    fun onTouchStart(x: Float, touches: List<ChartGestureTouch>, context: ChartGestureContext): ChartViewport? {
        if (touches.size >= 2) beginPinch(touches, context)
        else if (context.enablePan) beginPan(x, context)
        return null
    }

    fun onTouchMove(x: Float, touches: List<ChartGestureTouch>, context: ChartGestureContext): ChartViewport? {
        return when {
            touches.size >= 2 -> {
    ...
    fun onDoubleTap(context: ChartGestureContext): ChartViewport? {
    ...
    private fun updatePan(x: Float, context: ChartGestureContext): ChartViewport? {
        ...
        val deltaItems = ((start.x - x) / slotWidth).takeIf { it.isFinite() }?.roundToInt() ?: return null
        return LineViewportController.pan(context.itemCount, start.viewport, deltaItems)
    }
    ...
    private fun updatePinch(touches: List<ChartGestureTouch>, context: ChartGestureContext): ChartViewport? {
        ...
        return LineViewportController.zoom(
            itemCount = context.itemCount,
